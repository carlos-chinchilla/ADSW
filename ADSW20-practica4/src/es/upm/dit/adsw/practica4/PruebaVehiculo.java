...
import es.upm.dit.adsw.practica4.GrafoPosiciones;
import es.upm.dit.adsw.practica4.NavegadorImpl;
import es.upm.dit.adsw.practica4.Vector;
import es.upm.dit.adsw.practica4.Vehiculo;
...
public class PruebaVehiculo {
	...
	public void test1() {
		try {
			v.mover(new Vector(0,0),2);
			Vector destino = new Vector(30,0);
			Thread t=v.arrancaVehiculo(destino, 1);
			t.join();
			assertEquals("Vehiculo mal colocado ",destino,v.getPos());
		} catch(Exception e) {
			fail("Error ejecutando prueba test1 ");
		}
	}
	...
	public void test2() {
		try {
			v.mover(new Vector(0,0),2);
			Vector destino = new Vector(30,0);
			Thread t=v.arrancaVehiculo(destino, 2);
			t.join();
			assertEquals("Vehiculo mal colocado ",new Vector(0,0),v.getPos());
		} catch(Exception e) {
			fail("Error ejecutando prueba test2 ");
		}
	}
