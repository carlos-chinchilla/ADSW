...
public class Vehiculo {
	...
	private static Map<Vector,Map<Vector,ControlEntrada>> almacenControles=new HashMap<Vector,Map<Vector,ControlEntrada>>();
	...
	/**
	 * Devuelve el conjunto de detectores del vehiculo
	 * @return los detectores
	 * OJO EN EL CASO PREVIO SÖLO SE TENIA UN DETECTOR. ELEGIR UNO
	...
	public List<Vector> irA(Vector destino) {
		...
		while (!navegador.finMovimiento()) {
			...
			ControlEntrada ce=recuperaControlEntrada(c.desde(),c.hasta());
			// TODO Utilizar el ControlEntreda ce para evitar que dos vehiculos estén al mismo tiempo en un tramo
			ce.entrarEnTramo();
			getNavegador().getMapa().mueve(this, c);
			// TODO Hemos terminado de hacer el tramo, debemos actualizar ce dicendo que hemos salido
			ce.salirDeTramo();
			pasos.add(c.hasta());
	...
	/**
	 * Crea una hebra independiente que representa el movimiento del vehiculo. El vehiculo estara iendo y viniendo
	...
	public Thread arrancaVehiculo(Vector destino,int vueltas) throws Exception {
		if (getNavegador() == null) throw new Exception("solo podemos arrancar con el navegador inicializado");
		Thread t=new Thread(new Runnable() {
			@Override
			public void run() {
				Vector desde = getPos();
				Vector hasta = destino;
				try {
					for (int i=0; i < vueltas; i++) {
						Vehiculo.this.irA(hasta);
						// LOGGER.info("Soy "+Vehiculo.this.id+" y he llegado a "+Vehiculo.this.getPos());
						Object[] params= {Vehiculo.this.id,Vehiculo.this.getPos(),Vehiculo.this.getT()};
						LOGGER.log(Level.INFO, "Soy "+Vehiculo.this.id+" y he llegado a "+Vehiculo.this.getPos(),params);
						Vector tmp=hasta;
						hasta=desde;
						desde=tmp;
					}
				} catch(NoSuchElementException e) {
					System.err.println("No hay camino desde "+desde+" hasta "+hasta);
	...
	protected synchronized static ControlEntrada recuperaControlEntrada(Vector desde, Vector hasta) {
		Map<Vector,ControlEntrada> controlesh=almacenControles.get(hasta);
		if (controlesh == null) {
			controlesh=new HashMap<Vector,ControlEntrada>();
			almacenControles.put(hasta, controlesh);
		}
		Map<Vector,ControlEntrada> controlesd=almacenControles.get(desde);
		if (controlesd == null) {
			controlesd=new HashMap<Vector,ControlEntrada>();
			almacenControles.put(desde, controlesd);
		}
		ControlEntrada unControl=controlesd.get(hasta);
		if (unControl == null) {
			unControl=new ControlEntradaImpl();
			controlesd.put(hasta, unControl);
			controlesh.put(desde, unControl);
		}
		return unControl;
	}
	   ...
	   public static void main(String[] arg) {
			...
			*/
			...
			try {
				...
				Filter filter = new Filter() {
					...
					public boolean isLoggable(LogRecord record) {
						return record.getLevel() == Level.INFO 
								&& record.getMessage().contains(" entra en  ") 
				...
				Vehiculo v2=new Vehiculo("id001",new Vector(40.0,20.0),-1.0,new Vector(40.0,10.0),0.0);
				...
				Thread t2=v2.arrancaVehiculo(new Vector(0.0,0.0),2);
				Thread t3=v3.arrancaVehiculo(new Vector(40.0,0.0),8);
				t1.join();
				t2.join();
				t3.join();
